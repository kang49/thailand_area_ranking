...
def roc(n):
    ...
    for i in range(len(result)):
        if((result["Score"].values[i] >= n) and (pn_grid["pn"].values[i] == 1)):
            a += 1
        elif((result["Score"].values[i] >= n) and (pn_grid["pn"].values[i] == 0)):
            b += 1
        elif((result["Score"].values[i] < n) and (pn_grid["pn"].values[i] == 1)):
            c += 1
        else:
            d += 1
...
study_area = pd.read_csv('Input/Study-area.txt', sep='\t', names=["Lat", "Long", "Depth"], header=None)
...
result = study_area.assign(Score=0)
...
result.to_excel(r'Output.xlsx', sheet_name='Main', index=False)
